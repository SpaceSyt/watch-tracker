...
export function AuthForm({ mode }: AuthFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!email.trim() || !password.trim()) {
      setErrorMessage(dictionary.auth.required);
      return;
    }

    if (password.length < 6) {
      setErrorMessage(dictionary.auth.passwordLength);
      return;
    }
    ...
    try {
      ...
      setSuccessMessage(
        dictionary.auth.success,
      );
    } catch (error) {
      const message =
        error instanceof Error ? error.message : dictionary.auth.unexpected;
