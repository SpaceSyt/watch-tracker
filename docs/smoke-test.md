...
Use this checklist before merging feature work or before a deployment rehearsal. It intentionally covers the current MVP only: email/password auth, TMDB movie/TV search, title detail, saved-entry lifecycle, rating/review gating, remove from `/my`, and compact `/my` summaries.
...
- [ ] Confirm any visible helper text explains that ratings/reviews are available after moving to Watching or Completed.
